...
class DiscountService
{
    ...
    public function getAllProductDiscount($perPage = 10){
        return ProductDiscount::orderBy('id', 'desc')->paginate($perPage);
    }

    public function getAllProductDiscountActive($perPage = 6, $pageName = 'page')
    {
        return ProductDiscount::where('status', 1)->orderBy('id', 'desc')->paginate($perPage, ['*'], $pageName);
    }

    public function getAllProductDiscountActiveWithoutPagination($limit = 6)
    {
        return ProductDiscount::where('status', 1)->orderBy('id', 'desc')->limit($limit)->get();
    }
